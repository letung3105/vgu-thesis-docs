...
\begin{itemize}
    ...
    This allow for training with constant memory cost.
    \item \textbf{Adaptive computation}
    Modern \gls{ODE} solver guarantees on the growth of approximation error.
    ...
    Continuous transformations allow the change of variables formula become easier to compute.
    \item \textbf{Continuous time-series models}
    Continuously-defined dynamics can incorporate data that arrives at arbitrary times.
    ...
    \begin{algorithmic}
        \Function{ODEDerivative}{$\theta, t_0, t_1, z(t_1), \frac{\partial L}{\partial z(t_1)}$}
            ...
            \Comment{Define dynamics on augmented state}
                ...
                \Comment{Compute the vector-Jacobian project}
            \EndFunction
            ...
            \Comment{Solve reverse-time ODE}
